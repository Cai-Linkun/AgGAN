...
class Visualizer():
    def __init__(self, opt):
        ...
        if self.use_wandb:
            ...
            self.wandb_run._label(repo='AgGAN')
        ...
        if self.display_id > 0:  # show images in the browser using visdom
            ...
            if ncols > 0:        # show all the images in one visdom panel
                ...
                if label_html_row != '':
                    label_html += '<tr>%s</tr>' % label_html_row
                try:
                    _imgs = np.stack(images, 0)  # 3-dims N x W x H
                    _imgs = np.expand_dims(_imgs, 1)  # 4-dims N x 1 x W x H -> mutli single channel img
                    self.vis.images(
                        _imgs, nrow=ncols, win=self.display_id + 1, padding=2, opts=dict(title=title + " images")
                    )
                ...
                try:
                    for label, image in visuals.items():
                        image_numpy = image
                        self.vis.image(image, opts=dict(title=label), win=self.display_id + idx)
                        idx += 1
                except VisdomExceptionBase:
                    self.create_visdom_connections()
