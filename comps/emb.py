...
class RAC(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        d_cat = torch.cat([d1, d2, d3], dim=1)  # c * 3
        global_feat = self.global_pool(d_cat)  # (B, 3*out_channels, 1, 1)
        global_feat = global_feat.view(global_feat.shape[0], -1)  # Flatten to (B, 3*out_channels)

        weights = self.fc1(global_feat)  # (B, 64)
        weights = self.softmax(weights)  # Normalize to sum=1
